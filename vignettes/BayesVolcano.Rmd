...
If you used **rstan** or **brms** to fit your model, our `extract_fit()` function automatically converts your results into the right format. Just install the corresponding package (rstan or brms) first, then run `extract_fit(your_model)` to prepare your data.
...
The `prepare_volcano_df()` function takes as input the posterior and the annotation data frame. Run this now:
...
```{r}
d <- prepare_volcano_input(posterior = posterior,
...
This plot shows:
- X-axis: Posterior median effect size
- Y-axis: $\pi$-value
...
```{r,fig.width=7,fig.height=6}
plot_volcano(d, color = "group")
plot_volcano(d, color = "value")
```
